...
export default function ConfirmDialog({ title, message, confirmLabel = "Confirm", danger = true, busy = false, children, onConfirm, onCancel }) {
  ...
  return (
    <div
      ...
      onClick={onCancel}
    >
      <div className="agri-card" style={{ width: 380, maxWidth: "90vw", padding: 22, textAlign: "center" }} role="alertdialog" aria-label={title} onClick={(e) => e.stopPropagation()}>
        <div
          style={{
        ...
        <div className="agri-muted" style={{ fontSize: "0.85rem", marginBottom: children ? 12 : 20 }}>{message}</div>
        {children && <div style={{ marginBottom: 20 }}>{children}</div>}
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" className="btn btn-outline-secondary flex-fill" onClick={onCancel} disabled={busy}>Cancel</button>
          <button
            type="button"
